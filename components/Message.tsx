...
export const Message: React.FC<MessageProps> = ({ speaker, text, isPartial = false }) => {
  const isModel = speaker === 'model';

  return (
    <div className={`flex items-start gap-4 ${isModel ? '' : 'flex-row-reverse'}`}>
      <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${isModel ? 'bg-cyan-500' : 'bg-gray-600'}`}>
